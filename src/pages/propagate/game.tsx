...
export class Game {
      ...
      (data: any) => {
        data.features.forEach((country: any) => {
          if (country.geometry.type === 'Polygon') {
            const pointArr1: number[] = []
            const pointArr2: number[] = []
            ...
            shapeGroup.add(
              this.drawLineLoop(pointArr1, pointArr2, lineMaterial)
            )
  ...
  drawLineLoop = (
    ...
    geometry.setAttribute(
      'current',
      new THREE.BufferAttribute(new Float32Array(point1Arr), 3)
    )
  ...
  draw3DLine(elem: any, material: THREE.LineBasicMaterial) {
    ...
    const formXYZ = new THREE.Vector3().setFromSphericalCoords(
      100,
      fromPhi,
      fromTheta
    )
    const { phi: toPhi, theta: toTheta } = lon2phi(to[0], to[1])
    const toXYZ = new THREE.Vector3().setFromSphericalCoords(
      100,
      toPhi,
      toTheta
    )
    ...
    const point = this.drawFlyLine(
      points.map((point) => {
        return new THREE.Vector3(point.x, point.y, 0).applyQuaternion(
          startEndQua.quaternion
        )
      }),
      number,
      `${name}3d`
  ...
  drawFlyLine = (points: THREE.Vector3[], number: number, name: string) => {
    const positions: any[] = []

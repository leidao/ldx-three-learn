...
export class Ruler extends Object2D {
    ...
    drawXRuler(ctx) {
        ...
        const { camera: { position, zoom } } = scene;
        const { config } = this;
        // 绘制刻度线和刻度值
        // 计算 x 轴起点和终点范围
        ...
        while (startXInScene <= endXInScene) {
            ...
            for (let i = 1; i < 10; i++) {
                ...
                ctx.lineTo(x + (i * stepInScene * zoom) / 10, i === 5 ? y - config.h + 4 : y - config.h + 8);
            }
            ctx.stroke();
            ctx.closePath();
    ...
    drawYRuler(ctx) {
        ...
        while (startYInScene <= endYInScene) {
            ...
            for (let i = 1; i < 10; i++) {
                ...
                ctx.lineTo(i === 5 ? x - config.h + 4 : x - config.h + 8, y + (i * stepInScene * zoom) / 10);
            }
            ctx.stroke();
            ctx.closePath();

...
export class Game {
    ...
    useCss3Render(data) {
        ...
        // 创建控件对象
        this.controls = new OrbitControls(this.viewer.camera, this.labelRenderer.domElement);
        //监听鼠标、键盘事件
    ...
    createText(data, distance = 180) {
        ...
        data.forEach((elem) => {
            ...
            const pixel = threeToScreen(this.viewer.camera, this.viewer.container, vector);
            if (!elem.div) {
            ...
            elem.div.style.top = `${pixel.y - elem.div.clientHeight / 2
            // pixel.y - elem.div.clientHeight / 2 + (20 + size / 8 - distance / 12)
            }px`;
            if (name == '曲阜') {
                elem.div.style.left = `${pixel.x - elem.div.clientWidth / 2 + 15}px`;
                elem.div.style.top = `${pixel.y - elem.div.clientHeight / 2 + (40 + size / 8 - distance / 12)}px`;
            }
        });
    }
    update = () => {
